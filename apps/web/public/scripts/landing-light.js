...
window.CredXMotion.register(({on,request,top,clamp})=>{
 ...
 const specs=[...document.querySelectorAll('#hero,#about,#breakdown,#how-it-works,#progress,#modules')].map(host=>{
 ...
 on(window,'scroll',()=>{
  ...
  line.duration=plane===0?1400:plane===1?620:380;
 ...
 return {
  ...
  render(f){let active=false;
   specs.forEach(s=>s.lines.forEach(l=>{
    ...
    if(s.top+s.height<f.y||s.top>f.y+f.h){l.start=-Infinity;l.el.style.opacity='0';return;}
