...
window.CredXMotion.register(({clamp,range,top,on,request})=>{
 ...
 on(hero,'pointermove',e=>{if(e.pointerType!=='mouse'||innerWidth<768)return;tx=clamp(e.clientX/innerWidth)*2-1;ty=clamp(e.clientY/innerHeight)*2-1;request()});
 ...
 return {
  ...
  render(f){
   ...
   objects.forEach((el,i)=>{
    const d=poses[i],depth=f.mobile?.55:1,scale=d.scale*((f.mobile?.86:.8)+(f.mobile?.14:.2)*approach);
