...
try {
 ...
 async function prepare(page){if(engine==='webkit')await page.route('**/*',async r=>{if(r.request().resourceType()!=='document')return r.continue();const response=await r.fetch(),headers=response.headers();headers['content-security-policy']=headers['content-security-policy']?.replace('upgrade-insecure-requests','')||'';await r.fulfill({response,headers})});}
 for(const width of [375,390,430,768,1024,1280,1440,1920]){
  ...
  const about=await pos('#about'),h=width<768?844:1000;
  ...
  await scroll(about-100);assert.equal(await page.locator('.about-inner').evaluate(e=>getComputedStyle(e).opacity),'1');
  ...
  await page.goto(base+'#about',{waitUntil:'networkidle'});await page.waitForFunction(()=>getComputedStyle(document.querySelector('.about-inner')).opacity==='1');
